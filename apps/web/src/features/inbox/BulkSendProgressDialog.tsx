...
export const BulkSendProgressDialog: React.FC<BulkSendProgressDialogProps> = ({
  ...
  const startSending = async () => {
    ...
    for (let i = 0; i < sendItems.length; i++) {
      ...
      setSendItems((prev) =>
        prev.map((p, idx) => (idx === i ? { ...p, status: 'sending' as const } : p)),
      );
  ...
  const progress =
    sendItems.length > 0 ? ((successCount + errorCount) / sendItems.length) * 100 : 0;
        ...
        <List dense>
          {sendItems.map((item, index) => (
            <ListItem key={index}>
              ...
              <ListItemText primary={item.subject} secondary={item.recipient} />
              {item.error && (
      ...
      <DialogActions>
        {!sending && !completed && (
          <>
            ...
            <Button variant="contained" onClick={startSending} startIcon={<Send />}>
