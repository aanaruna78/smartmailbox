                ...
                <form onSubmit={handleSubmit}>
                    ...
                    {testResults && (
                        <Box sx={{ mt: 3, p: 2, bgcolor: 'background.default', borderRadius: 1 }}>
                            <Typography variant="subtitle2" gutterBottom>Connection Test Results:</Typography>
                            <Stack spacing={1}>
                                {testResults.imap && (
                                    <Box sx={{ display: 'flex', alignItems: 'center', color: testResults.imap.success ? 'success.main' : 'error.main' }}>
                                        {testResults.imap.success ? <CheckCircle fontSize="small" sx={{ mr: 1 }} /> : <ErrorIcon fontSize="small" sx={{ mr: 1 }} />}
                                        <Typography variant="body2">
                                            IMAP: {testResults.imap.message}
                                        </Typography>
                                    </Box>
                                )}
                                {testResults.smtp && (
                                    <Box sx={{ display: 'flex', alignItems: 'center', color: testResults.smtp.success ? 'success.main' : 'error.main' }}>
                                        {testResults.smtp.success ? <CheckCircle fontSize="small" sx={{ mr: 1 }} /> : <ErrorIcon fontSize="small" sx={{ mr: 1 }} />}
                                        <Typography variant="body2">
                                            SMTP: {testResults.smtp.message}
                                        </Typography>
                                    </Box>
                                )}
                            </Stack>
