...
// APIs
export const generateDraftJob = async (id: number, instructions: string, tone: string): Promise<{ job_id: number, status: string }> => {
...
export const updateDraft = async (draftId: number, content: string, is_accepted?: boolean): Promise<Draft> => {
    const response = await http.put(`/drafts/${draftId}`, { content, is_accepted });
    return response.data;
};

// Send Email
export const sendEmail = async (emailId: number, recipient: string, subject: string, body_html?: string, body_text?: string): Promise<{ message: string, job_id: number }> => {
    const response = await http.post(`/emails/${emailId}/send`, { recipient, subject, body_html, body_text });
    return response.data;
};
...
// Bulk Actions
export const bulkGenerateDrafts = async (emailIds: number[], instructions: string, tone: string): Promise<{ message: string, job_id: number }> => {
